...
def detect_separator(unit_body: str) -> str:
    ...
    def count_outside_parens(pattern):
        # Remove content inside parentheses for counting
    ...
    # Only count numbered list markers at line starts, not decimals in course codes
    numbered_count = len(re.findall(r'(?:^|\n)\s*\d+\.\s+', re.sub(r'\([^)]*\)', '', cleaned)))
    bullet_count = count_outside_parens(r'[•·◦]')
    
    # Count newlines (assuming unit_body has them preserved)
    ...
    # If no separators detected, default to newline
    ...
    # Prefer certain separators when counts are equal
    candidates = [k for k, v in counts.items() if v == max_count]
    if len(candidates) > 1:
        # Prefer newline over comma, then dash, etc.
...
def parse_syllabus(text: str) -> Dict:
    """
    Parses syllabus text into structured JSON.
    """
    ...
    units = []
    for i in range(1, len(units_text), 2):  # Skip the headers, process content
...
def extract_course_name(text: str) -> str:
    ...
    for line in lines[:10]:  # Check first 10 lines
        line = line.strip()
        if re.search(r'[A-Z]{2,3}\d{4,6}', line):  # Near course code
            # Extract the descriptive part before or after code
...
def extract_semester(text: str) -> str:
    ...
    # Look for Roman numerals or numbers
...
def parse_unit(header: str, body: str) -> Dict:
    ...
    # If name is still "Unknown" or empty, try to extract from first line of body
    if unit_name == "Unknown" or not unit_name:
        ...
        if body_lines:
            ...
            if len(first_line) > 5 and not re.match(r'^\d', first_line):
                ...
                body = '\n'.join(body_lines[1:])  # Remove first line from body
    
    # Stop before hard boundaries
    stop_patterns = [
    ...
    # Detect separator and split topics
    ...
    for t in topic_strings:
        if is_valid_topic(t):
            ...
            if cleaned["name"]:  # Only add if name is not empty
...
def split_topics(body: str, separator: str) -> List[str]:
    if separator == 'newline':
        ...
        # Handle various dash types and clean up
        # But don't split on dashes if they're inside parentheses
        topics = []
        current_topic = ""
        paren_depth = 0
        ...
        # If no topics found with dash splitting, try comma splitting
        ...
        # Split only on explicit list markers at the start of a line
        if not re.search(r'(?:^|\n)\s*\d+\.\s+', body):
            return [body.strip()]
        topics = re.split(r'(?:^|\n)\s*\d+\.\s+', body)
...
def looks_like_table_row(topic: str) -> bool:
    ...
    # Reject rows that are mostly numeric or table-style metadata
    ...
    # Reject common average/table footers with many numeric values
    ...
    # Reject course-code-like rows with trailing numbers
...
def clean_topic(topic: str) -> Dict[str, any]:
    # Remove leading course-code-like prefixes and noisy row prefixes
...
def extract_topic_and_subtopics(line: str) -> tuple[str, list[str]]:
    """Extract topic name and subtopics from a line using ':' as delimiter or parentheses."""
    line = line.strip()
    
    # Check for parentheses subtopics
    if '(' in line and ')' in line:
        # Extract content within parentheses as subtopics
...
def _split_subtopics(subtopic_text: str) -> list[str]:
    """Split subtopic text into individual subtopics."""
    ...
    for sub in subtopics:
        ...
        sub = re.sub(r'[-–—\s]+$', '', sub)
        if len(sub) >= 3:  # Only keep meaningful subtopics (changed from > 3 to >= 3)
            cleaned.append(sub)
    
    return cleaned

def calculate_confidence(body: str, topics: List[str]) -> float:
    # Simple confidence based on topic count and body length
    if not topics:
        return 0.0
    avg_topic_length = sum(len(t) for t in topics) / len(topics)
    if avg_topic_length < 10:
        return 0.5  # Short topics might be noise
    if len(topics) < 3:
        return 0.6  # Few topics
    return 0.9  # Good parse